...
import { useNFTStore } from "@bitcoinbaby/core";
...
export default function NFTsPage() {
  ...
  const handleMint = useCallback(async () => {
    ...
    if (result.success && result.nft) {
      // Add to owned NFTs (in real app, this would come from chain)
      if (isDemo) {
  ...
  // Use demo or real NFTs
  const nfts = isDemo ? demoNFTs : ownedNFTs;

  // Evolution handler (demo mode only)
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="font-pixel text-xl md:text-2xl text-pixel-primary mb-2">
                GENESIS BABIES
              </h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/wallet"
                className="font-pixel text-[8px] text-pixel-primary hover:text-pixel-secondary transition-colors"
              >
                WALLET
              </Link>
            </div>
        ...
        {!wallet && !isDemo && (
          <div className="mb-8 p-4 bg-pixel-bg-medium border-4 border-pixel-warning text-center">
            ...
            <Link
              href="/wallet"
              className="inline-block font-pixel text-[8px] uppercase px-4 py-2 bg-pixel-primary text-pixel-text-dark border-4 border-black shadow-[4px_4px_0_0_#000] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_#000] transition-transform"
            >
              Connect Wallet
            </Link>
          </div>
        ...
        {/* Footer */}
        <footer className="mt-12 pt-8 border-t-2 border-pixel-border text-center">
          <p className="font-pixel text-[7px] text-pixel-text-muted uppercase">
            Genesis Babies NFTs powered by Charms Protocol
          </p>
